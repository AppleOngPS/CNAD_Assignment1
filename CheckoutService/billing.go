...
// Database connection (assuming `db` is set up elsewhere in your code)
...
// Function to fetch available promo codes and their discount
func getPromoCodes() ([]PromoCode, error) {
	rows, err := db.Query(`
		SELECT promotionCode, discount
		FROM promotion`)
	if err != nil {
	...
	for rows.Next() {
		...
		promoCodes = append(promoCodes, PromoCode{Code: code, Discount: discount})
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("error iterating over promo codes: %v", err)
	}
