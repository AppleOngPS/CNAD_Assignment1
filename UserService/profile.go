...
func updateProfile(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		if password != "" {
			...
			// Update the user's details in the database with the hashed password
			_, err = db.Exec("UPDATE users SET username = ?, email = ?, password = ? WHERE userID = ?", username, email, string(hashedPassword), userID)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
		} else {
			// If password is empty, just update the other fields
			_, err := db.Exec("UPDATE users SET username = ?, email = ? WHERE userID = ?", username, email, userID)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
